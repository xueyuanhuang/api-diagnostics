...
export async function listAnimations(env: Storage, userId: string, cursor?: string | null): Promise<AnimationSummary[]> {
  ...
  const rows = await env.DB.prepare('SELECT id, model, created_at FROM animation_results WHERE user_id = ? AND (created_at < ? OR (created_at = ? AND id < ?)) ORDER BY created_at DESC, id DESC LIMIT 100').bind(userId, before, before, lastId || '\uffff').all<{ id: string; model: string; created_at: number }>();
  return rows.results.map(row => ({ id: row.id, model: row.model, savedAt: new Date(row.created_at).toISOString() }));
}
...
export async function saveAnimation(env: Storage, userId: string, animation: SavedAnimation) {
  const existing = await env.DB.prepare('SELECT id, model, created_at FROM animation_results WHERE user_id = ? AND id = ?').bind(userId, animation.id).first<{ id: string; model: string; created_at: number }>();
  if (existing) return { id: existing.id, model: existing.model, savedAt: new Date(existing.created_at).toISOString() };
  ...
  await env.DB.prepare('INSERT INTO animation_results (user_id, id, model, evidence_key, created_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(user_id, id) DO NOTHING').bind(userId, animation.id, animation.model, key, createdAt).run();
  return { id: saved.id, model: saved.model, savedAt: saved.savedAt };
}
