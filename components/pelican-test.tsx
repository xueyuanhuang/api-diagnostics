      ...
      <div className="mx-auto max-w-[1480px] px-4 py-6 sm:px-6 lg:px-10">
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[360px_minmax(0,1fr)]">
          <form onSubmit={start} className="space-y-5 rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <p className="text-sm leading-6 text-muted-foreground">Your key is sent through this site's relay to your provider for this request. This test does not save your key or results.</p>
          ...
          <div className="min-w-0 space-y-5">
            ...
            {result && <section className="space-y-4 rounded-2xl border border-border bg-card p-5">
              <p className="break-words text-sm text-muted-foreground">Model: {result.returnedModel || model} · Input: {result.totalInputTokens ?? '—'} tokens · Output: {result.outputTokens ?? '—'} tokens · {result.totalTimeMs == null ? '—' : (result.totalTimeMs / 1000).toFixed(1)} seconds</p>
