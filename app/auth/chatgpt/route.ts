...
export async function GET(request: Request) {
  ...
  if (!user || !user.userId.startsWith('google:')) return redirect('/auth/google?return_to=%2Fauth%2Fchatgpt');
  ...
  await env.DB.batch([
    ...
    env.DB.prepare('INSERT INTO auth_flows (token_hash,state,verifier,nonce,return_to,expires_at) VALUES (?,?,?,?,?,?)')
      .bind(await tokenHash(token),state,verifier,`import:${user.userId}`,safeRelativeReturnPath(new URL(request.url).searchParams.get('return_to')??'/connections'),Date.now()+600000),
  ]);
